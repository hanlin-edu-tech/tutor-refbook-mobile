...
let bucketNameForTest = "tutor-test-events";
...
let copyStaticTask = dest => {
    return () => {
        return gulp
            .src(["src/**/*.html", "src/css/**/*.css", "src/css/icon/*", "src/css/icon/*.ico", "src/lib/**/*", "src/js/*.js", "src/img/*.png", "src/img/*.jpg"], {
                base: "src"
            })
            .pipe(gulp.dest(dest));
    };
...
let productionChangeToTest = () => {
    ...
    return gulp
        ...
        .pipe(replace(regExp, /\/(event-collection_107)\/(\d\.\d\.\d{1,2}-SNAPSHOT)/g));
};
...
let uploadGCS = bucketName => {
    return gulp
        .src(["./dist/*.html", "./dist/css/**/*.css", "./dist/js/**/*.js", "./dist/lib/**/*.@(js|json|css|map|svg|eot|ttf|woff|woff2)", "./dist/img/**/*.@(png|jpg|svg|gif)"], {
            base: `${__dirname}/dist/`
        })
        .pipe(
            gcPub({
